...
#Do the git-ing
cmd='git commit -a -m "Commit before run create_U_list_JLD" > /dev/null'
print(cmd)
...
open('create_U_list_JLD.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('create_U_list_JLD.task','w')
template_file='create_U_list_JLD.template'
...
for L in xrange(8):
	...
	task_file.write('bash create_U_list_JLD_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('create_U_list_JLD.sbatch.template','r').read()
...
open('create_U_list_JLD.sbatch','w').write(contents)
