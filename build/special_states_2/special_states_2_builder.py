...
#Do the git-ing
cmd='git commit -a -m "Commit before run special_states_2" > /dev/null'
print(cmd)
...
open('special_states_2.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('special_states_2.task','w')
template_file='special_states_2.template'
...
for L in xrange(32):
	...
	task_file.write('bash special_states_2_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('special_states_2.sbatch.template','r').read()
...
open('special_states_2.sbatch','w').write(contents)
