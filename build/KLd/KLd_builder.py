...
#Do the git-ing
cmd='git commit -a -m "Commit before run KLd" > /dev/null'
print(cmd)
...
for L in xrange(64):
	...
	task_file.write('bash KLd_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('KLd.sbatch.template','r').read()
...
open('KLd.sbatch','w').write(contents)
