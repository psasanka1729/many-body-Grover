...
#Do the git-ing
cmd='git commit -a -m "Commit before run special_states_h_eff" > /dev/null'
print(cmd)
...
open('special_states_h_eff.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('special_states_h_eff.task','w')
template_file='special_states_h_eff.template'
...
for L in xrange(64):
	...
	task_file.write('bash special_states_h_eff_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('special_states_h_eff.sbatch.template','r').read()
...
open('special_states_h_eff.sbatch','w').write(contents)
