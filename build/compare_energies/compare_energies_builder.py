...
#Do the git-ing
cmd='git commit -a -m "Commit before run compare_energies" > /dev/null'
print(cmd)
...
open('compare_energies.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('compare_energies.task','w')
template_file='compare_energies.template'
...
for L in xrange(64):
	...
	task_file.write('bash compare_energies_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('compare_energies.sbatch.template','r').read()
...
open('compare_energies.sbatch','w').write(contents)
