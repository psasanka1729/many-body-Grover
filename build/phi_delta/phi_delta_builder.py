...
#Do the git-ing
cmd='git commit -a -m "Commit before run phi_delta" > /dev/null'
print(cmd)
...
open('phi_delta.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('phi_delta.task','w')
template_file='phi_delta.template'
...
contents=open('phi_delta.sbatch.template','r').read()
...
open('phi_delta.sbatch','w').write(contents)
