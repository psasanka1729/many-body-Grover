...
#Do the git-ing
cmd='git commit -a -m "Commit before run h_eff_level_statistics" > /dev/null'
print(cmd)
...
open('h_eff_level_statistics.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('h_eff_level_statistics.task','w')
template_file='h_eff_level_statistics.template'
...
for L in xrange(32):
	...
	task_file.write('bash h_eff_level_statistics_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('h_eff_level_statistics.sbatch.template','r').read()
...
open('h_eff_level_statistics.sbatch','w').write(contents)
