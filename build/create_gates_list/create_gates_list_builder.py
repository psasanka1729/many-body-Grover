...
#Do the git-ing
cmd='git commit -a -m "Commit before run create_gates_list" > /dev/null'
print(cmd)
...
open('create_gates_list.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('create_gates_list.task','w')
template_file='create_gates_list.template'
...
for L in xrange(4):
	...
	task_file.write('bash create_gates_list_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('create_gates_list.sbatch.template','r').read()
...
open('create_gates_list.sbatch','w').write(contents)
