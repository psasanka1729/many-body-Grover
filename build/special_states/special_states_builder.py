...
#Do the git-ing
cmd='git commit -a -m "Commit before run special_states" > /dev/null'
print(cmd)
...
open('special_states.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('special_states.task','w')
template_file='special_states.template'
...
for L in xrange(128):
	...
	task_file.write('bash special_states_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('special_states.sbatch.template','r').read()
...
open('special_states.sbatch','w').write(contents)
