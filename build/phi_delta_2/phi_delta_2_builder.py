...
#Do the git-ing
cmd='git commit -a -m "Commit before run phi_delta_2" > /dev/null'
print(cmd)
...
open('phi_delta_2.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('phi_delta_2.task','w')
template_file='phi_delta_2.template'
...
for L in xrange(48):
	...
	task_file.write('bash phi_delta_2_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('phi_delta_2.sbatch.template','r').read()
...
open('phi_delta_2.sbatch','w').write(contents)
