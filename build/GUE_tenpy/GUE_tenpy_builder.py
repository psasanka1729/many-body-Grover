...
#Do the git-ing
cmd='git commit -a -m "Commit before run GUE_tenpy" > /dev/null'
print(cmd)
...
for L in xrange(64):
	...
	task_file.write('bash GUE_tenpy_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('GUE_tenpy.sbatch.template','r').read()
...
open('GUE_tenpy.sbatch','w').write(contents)
