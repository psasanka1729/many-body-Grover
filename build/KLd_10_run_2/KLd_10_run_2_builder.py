...
partition_info=['smallmem',16] # = [partition,ncores]
...
#Do the git-ing
cmd='git commit -a -m "Commit before run KLd_10_run_2" > /dev/null'
print(cmd)
...
open('KLd_10_run_2.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('KLd_10_run_2.task','w')
template_file='KLd_10_run_2.template'
...
for L in xrange(64):
	...
	task_file.write('bash KLd_10_run_2_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('KLd_10_run_2.sbatch.template','r').read()
...
open('KLd_10_run_2.sbatch','w').write(contents)
