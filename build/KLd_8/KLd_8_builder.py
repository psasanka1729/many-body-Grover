...
#Do the git-ing
cmd='git commit -a -m "Commit before run KLd_8" > /dev/null'
print(cmd)
...
for L in xrange(64):
	...
	task_file.write('bash KLd_8_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('KLd_8.sbatch.template','r').read()
...
open('KLd_8.sbatch','w').write(contents)
