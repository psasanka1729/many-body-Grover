...
#Do the git-ing
cmd='git commit -a -m "Commit before run h_eff_spectrum" > /dev/null'
print(cmd)
...
open('h_eff_spectrum.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('h_eff_spectrum.task','w')
template_file='h_eff_spectrum.template'
...
for L in xrange(64):
	...
	task_file.write('bash h_eff_spectrum_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('h_eff_spectrum.sbatch.template','r').read()
...
open('h_eff_spectrum.sbatch','w').write(contents)
