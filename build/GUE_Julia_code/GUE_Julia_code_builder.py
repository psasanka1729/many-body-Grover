...
#Do the git-ing
cmd='git commit -a -m "Commit before run GUE_Julia_code" > /dev/null'
print(cmd)
...
open('GUE_Julia_code.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('GUE_Julia_code.task','w')
template_file='GUE_Julia_code.template'
...
for L in xrange(64):
	...
	task_file.write('bash GUE_Julia_code_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('GUE_Julia_code.sbatch.template','r').read()
...
open('GUE_Julia_code.sbatch','w').write(contents)
