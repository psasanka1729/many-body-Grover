...
#Do the git-ing
cmd='git commit -a -m "Commit before run probability_time_2" > /dev/null'
print(cmd)
...
open('probability_time_2.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('probability_time_2.task','w')
template_file='probability_time_2.template'
...
for L in xrange(16):
	...
	task_file.write('bash probability_time_2_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('probability_time_2.sbatch.template','r').read()
...
open('probability_time_2.sbatch','w').write(contents)
