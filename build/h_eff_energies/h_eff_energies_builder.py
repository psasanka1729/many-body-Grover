...
#Do the git-ing
cmd='git commit -a -m "Commit before run h_eff_energies" > /dev/null'
print(cmd)
...
open('h_eff_energies.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('h_eff_energies.task','w')
template_file='h_eff_energies.template'
...
for L in xrange(16):
	...
	task_file.write('bash h_eff_energies_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('h_eff_energies.sbatch.template','r').read()
...
open('h_eff_energies.sbatch','w').write(contents)
