...
#Do the git-ing
cmd='git commit -a -m "Commit before run beta" > /dev/null'
print(cmd)
...
for L in xrange(128):
	...
	task_file.write('bash beta_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('beta.sbatch.template','r').read()
...
open('beta.sbatch','w').write(contents)
